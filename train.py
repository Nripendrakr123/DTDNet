...
class TrainModule(object):
    ...
    def train_network(self, args):
        self.optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), args.init_lr )
    ...
    def run_epoch(self, phase, data_loader, criterion):
        ...
        for data_dict in data_loader:
            print('it --> {0}  , total-->{1} '.format(it , len(data_loader)) ,end='\r', flush=True)
            it = it + 1

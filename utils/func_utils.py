...
def non_maximum_suppression(pts, scores):
    ...
    keep_index = NMS_numpy_bbox(nms_item, nms_thresh=0.1)
...
def write_results(args,
    ...
    for index in range(len(dsets)):
        ...
        label_det = data_dict['annotation']['pts']
        label_cat = data_dict['annotation']['cat']
        gt_item = np.concatenate([ label_cat[:, np.newaxis] , label_det[:, 0:1, 0],
                                   label_det[:, 0:1, 1],
                                   label_det[:, 1:2, 0],
                                   label_det[:, 1:2, 1],
                                   label_det[:, 2:3, 0],
        ...
        for cat in dsets.category:
            ...
            if pts_cat.shape[0]:
                ...
                pred_list.append(nms_results)
                results[cat][img_id].extend(nms_results)

        # matrix_obj.process_batch(nms_results.astype(np.int64), gt_item)
        matrix_obj.process_batch(np.asarray(pred_list).astype(np.int64)[0], gt_item)
        if print_ps:
            print('testing {}/{} data {}'.format(index+1, len(dsets), img_id))
    matrix_obj.plot()
    print("tp_fp",matrix_obj.tp_fp())
    matrix_obj.print()
    for cat in dsets.category:
        if cat == 'background':
            continue

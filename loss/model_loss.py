...
class BCELoss(nn.Module):
    ...
    def _tranpose_and_gather_feat(self, feat, ind):
        '''
                model output : [ B , output_feature(10) , H , W ]
                GT : output    [ B , N , output_feature(10)]

                Convert Model output to GT output
                [ B , output_feature , H , W ] --(permute)--> [ B , H , W , output_feature ] --(view)--> [ B , HxW , output_feature]
                                                                                                                   |
                                                                                                                   |
                                                                                                                   V
    ...
    def forward(self, output, mask, ind, target):
        ...
        if mask.sum():
            ...
            loss = F.binary_cross_entropy(pred.masked_select(mask),target.masked_select(mask),reduction='mean')
            return loss
        else:
            return 0.
...
if __name__ == '__main__':
    ...
    print(topk_clses , topk_clses.shape)
    topk_inds = _gather_feat(topk_inds.view(batch, -1, 1), topk_ind).view(batch, K)
    print("*" * 50)
    print(topk_ind)
    print(topk_ys.view(batch, -1, 1))
